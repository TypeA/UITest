...
public class Photo extends DatabasesData {
    ...
    public String getRandomPhotoIdWithSizeAndSecurity(String user, String security, String size) {
        ...
        String select = "select photo_id from lj_c" 
                + clusterId + ".fotki_photo_prop where userid=" + userId + " and "
        ...
        String select1 = "Select photo_id from lj_c" + clusterId 
                + ".fotki_photos where userid = " + userId
                + " and security = '" + security 
                + "' and album_id!=71 and photo_id in(" 
                + String.join(", ", ans) + ")";
    ...
    public String getAlbumContainsPhotoId(String photoId, String user) {
        String userId = userData().getUserId(user);
        String clusterId = userData().getUserClusterId(user);
        String select = "Select album_id from lj_c"
                + clusterId + ".fotki_photos where userid = " 
                + userId + " and "
                + "photo_id=" + photoId;
        String albumId = workWithDB()
                ...
                .get(0)
                .get(0);
        String select1 = "Select album_title from lj_c" 
                + clusterId + ".fotki_albums where userid=" 
    ...
    public String getPhotoIdInUrl(String user, String photoId) {
        String select = "Select photo_id*256+auth from lj_c"
                + userData().getUserClusterId(user) + ".fotki_photos where userid=" 
                + userData().getUserId(user) + " and photo_id=" + photoId;
        return workWithDB()
                .conect()
